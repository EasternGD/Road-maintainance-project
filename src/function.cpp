...
void PopulationType::crossover()
{
	...
	while (i < POPULATION_CNT)
	{
		...
		if (r < CROSSOVER_RATE)
		{
			...
			//p2 = rand() % (row - 1) + 1;
			//p1 = rand() % p2;
			//offspring[i].binaryValue = pool[t1].binaryValue.substr(0, p1) + pool[t2].binaryValue.substr(p1, p2) + pool[t1].binaryValue.substr(p2, row);
			//offspring[i].cost = 0;
			//offspring[i].benefit = 0;
			// cout << endl;
		}
...
void saveJSON(const PopulationType &p1, const PopulationType &p2, string fileName)
{
	...
	{
		sum[0] += p1.BestOne.cost + p2.BestOne.cost;
		sum[1] += p1.BestOne.benefit + p2.BestOne.benefit;
		//fileOutput << "[" << endl;
		fileOutput << "\t{" << endl;
		fileOutput << "\t\t\"NUMBER\":[\"";
